...
namespace LittleWhales.Infrastructure
{
    public static partial class Extention
    {
        ...
        /// <returns></returns>
        public static string ToFieldAsName(this string input, Dictionary<string, string> fieldAsNames)
        {
            if (string.IsNullOrEmpty(input) || fieldAsNames == null || fieldAsNames.Count <= 0)
            {
                return input;
            }

            foreach (KeyValuePair<string, string> fieldAsName in fieldAsNames.Where(fieldAsName => fieldAsName.Key.Equals(input)))
            {
                return string.IsNullOrEmpty(fieldAsName.Value) ? fieldAsName.Key : fieldAsName.Value;
            }
            ...
            foreach (var item in type.GetProperties())
            {
                ...
                foreach (Match match in Regex.Matches(json, pattern))
                {
                    switch (item.PropertyType.ToString())
                    {
                        ...
                        case "System.Boolean": info.SetValue(obj, Convert.ToBoolean(match.Groups[1].ToString()), null); ; break;
                    }
                }
            }
            return (T)obj;
